...
Input must always have shape (3, h, w)
"""
import random
import numpy as np 
import PIL
...
def resize(image, target, size, max_size=None):
    ...
    ratios = tuple(
        float(s) / float(s_orig) for s, s_orig in zip(rescaled_image.shape[1:], image.shape[1:])
    )
...
class Resize(object):
    ...
    def __call__(self, img, target=None):
        return resize(img, target, self.min_size, self.max_size)
...
class Normalize(object):
    def __init__(self, mean = [0.485, 0.456, 0.406], std = [0.229, 0.224, 0.225], reparam=True):
        self.mean = mean
        self.std = std
...
class Resizer(object):
    ...
    def __call__(self, image, target, min_side=608, max_side=1024):
