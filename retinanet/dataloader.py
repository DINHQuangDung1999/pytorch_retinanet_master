...
class CocoDataset(Dataset):
    ...
    def __init__(self, root_dir, set_name='train2017', is_zsd = True, transform=None):
    ...
    def load_classes(self):
        ...
        if self.is_zsd == True:
            print(f'Mode: zsd')
            if os.path.split(self.root_dir)[-1] == 'DOTA':
                self.seen_ids = [1,2,4,6,7,8,9,10,11,12,14,15,16]
                self.unseen_ids = [3,5,13] # storage-tank, tennis-court, swimming-pool
            elif os.path.split(self.root_dir)[-1] == 'PascalVOC':
                self.seen_ids = [1,2,3,4,5,6,8,9,10,11,13,14,15,16,17,20]
                self.unseen_ids = [7, 12, 18, 19] # car, dog, sofa, train
            elif os.path.split(self.root_dir)[-1] == 'MSCOCO':
                # pass
                self.seen_ids = [1, 2, 3, 4, 6, 8, 9, 10, 11, 13, 15, 16, 18, 19, 20, 21, 22, 24, 25, 27, 28, 
            ...
            for i, id in enumerate(self.seen_ids + self.unseen_ids):
                self.coco_labels[i] = id
                self.coco_labels_inverse[id] = i
            ...
            self.labels              = {}
            for c in categories: 
                self.classes[c['name']] = c['id'] # load class names (name -> label)
                self.labels[c['id']] = c['name']  # also load the reverse (label -> name)
